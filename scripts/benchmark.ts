/**
 * Benchmark: JSON vs CSV (full) vs CSV (optimized)
 *
...
function kb(v: number) {
  return v.toFixed(1).padStart(8) + " KB";
}
function ms(v: number) {
  return v.toFixed(3).padStart(9) + " ms";
}
...
function printReport(r: FileReport) {
  ...
  for (const { label, key } of rows) {
    ...
    console.log(
      `     CSV full      ${fmt(r.csvFull[key])}${badge(r.json[key], r.csvFull[key])}`,
    );
    console.log(
      `     CSV optimized ${fmt(r.csvOpt[key])}${badge(r.json[key], r.csvOpt[key])}`,
    );
  }
}
...
function printSummary(reports: FileReport[]) {
  ...
  console.log(sep2);
  console.log(
    `\n  ${"File".padEnd(18)} ${"JSON".padStart(9)} ${"CSV full".padStart(9)} ${"CSV opt".padStart(9)}  ${"Saved vs JSON".padStart(14)}`,
  );
  ...
  if (bar) {
    ...
    console.log(
      `     CSV full:      ${ms(bar.csvFull.decompressParseMs)}${badge(bar.json.decompressParseMs, bar.csvFull.decompressParseMs)}`,
    );
    console.log(
      `     CSV optimized: ${ms(bar.csvOpt.decompressParseMs)}${badge(bar.json.decompressParseMs, bar.csvOpt.decompressParseMs)}`,
    );
  }
  console.log();
}
...
console.log(
  "  JSON (source)  vs  CSV full (naive)  vs  CSV optimized (shipped)\n",
);
